...
function detectOS() {
  ...
  let macOSVersion = ""; // For storing macOS version
  ...
  if (userAgent.indexOf("Windows NT 10.0") !== -1) {
    ...
    if (userAgent.indexOf("Ubuntu") !== -1) {
      ...
      distro = "Debian";
    } else if (userAgent.indexOf("Arch") !== -1 || userAgent.indexOf("arch") !== -1) {
      distro = "Arch Linux";
  ...
  return { os, arch, distro, macOSVersion };
}

// Display the detected OS, Distro (if Linux), macOS version (if macOS), and Architecture
const { os, arch, distro, macOSVersion } = detectOS();
let osInfoText = `You are running ${os} (${arch} architecture).`;

if (os === "macOS" && macOSVersion) {
  osInfoText = `You are running ${macOSVersion} with ${arch} architecture.`;
} else if (os === "Linux" && distro) {
